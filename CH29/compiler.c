...
static uint_8 fieldConstant(Token* className, Token* field) {        //challenge 1 ch29
...
static void dot(bool canAssign) {
  ...
  bool wasThis = thisAccess;      //challenge 1 ch 29
  thisAccess = false;             //challenge 1 ch 29
  ...
  if (canAssign && match(TOKEN_EQUAL)) {
    ...
    uint8_t name = mangle ? fieldConstant(&currentClass->name, &field) : makeConstatn(OBJ_VAL(copyString(field.start, field.length)));    //challenge 1 ch 29
    
    emitBytes(OP_SET_PROPERTY, name);
  } else if (match(TOKEN_LEFT_PAREN)) {

    if (!plainBuilt) {        //challenge 1 ch 29
